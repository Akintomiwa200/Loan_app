...
const BigCard2 = ({ title, mainValue, subValue, backgroundColor, isSelected, onClick }) => {
    return (
        <div
            className={`${styles.main} ${isSelected ? styles.selected : ''}`}
            style={{ backgroundColor }}
            onClick={onClick} // Trigger the click event to handle selection
        >
            <div>
                <div>
                    <h6 className={styles.tit}>Name</h6>
...
BigCard2.propTypes = {
    ...
    subValue: PropTypes.string.isRequired,
    backgroundColor: PropTypes.string,
    isSelected: PropTypes.bool, // New prop to determine if selected
    onClick: PropTypes.func, // Event handler for clicking the card
};
